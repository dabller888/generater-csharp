...
namespace NVelocity.Runtime.Parser.Node
{
	...
	public class MapGetExecutor : AbstractExecutor
    {
        ...
        protected internal virtual void Discover(System.Type clazz)
        {
            ...
            {
				bool isAssignableToGenericDictionary = typeof(IDictionary).IsAssignableFrom(clazz);

				if (isAssignableToGenericDictionary)
                {
                    try
                    {
                        Property = new PropertyEntry(typeof(IDictionary).GetProperty("Item"), null);
                    }
                    /**
